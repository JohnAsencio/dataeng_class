...
def row2vals(row):
    for key in row:
        if row[key] == "" or row[key] == None:
            row[key] = 0  # Replace empty strings with None
        elif not row[key]:
            row[key] = 0  # Handle other null values as needed
...
#        """) """
###################################
# CODE FOR COPY FROM (Faster way to load data)
##################################
    with conn.cursor() as cursor:
        start = time.perf_counter()
